...
usage = "qcdutils_boot.py *.log 'x[<a>]/y[<b>]' 'abs(a-b)==1'\n" \
...
def test_ibootstrap():
    ...
    for i in range(100):
        ...
        for t in range(16):
            for t1 in range(16):
               file.write('C3[%.2i][%.2i]: %f\n' % (t,t1,(4.0+random.gauss(0,0.05*(2+t+t1)))*exp(-0.2*(t+t1))))
    file.close()
    IBootstrap('test_samples.log','"C3[<t1>][<t2>]"/"C2[<t1>]"/"C2[<t2>]"','t1==t2',0,0,100).log_report()    
    return 0
